...
const Header = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 bg-neutral-background py-6 px-6 sm:px-12 z-[1000]">
      ...
      >
        <nav className="flex flex-col space-y-[0.25rem] py-4 px-4">
          {[
            { path: '/', label: 'Home' },
            { path: '/about-us', label: 'About Us' },
            { path: '/our-initiative', label: 'Our Initiative' },
          ...
          {['/donate-amount', '/donate-items'].map((link, i) => (
            ...
            >
              <Link to={link} onClick={toggleMobileMenu}>
                <button className="w-full bg-primary-blue text-white py-[0.75rem] rounded-[20px] font-semibold text-[0.9rem] shadow-custom-donate">
